class FlowchartsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @flowchart.save
        ...
        format.json { render :show, status: :created, location: @flowchart }
      else
  ...
  def update
    respond_to do |format|
      if @flowchart.update(flowchart_params)
        ...
        format.json { render :show, status: :ok, location: @flowchart }
      else
  ...
  private
    ...
    def flowchart_params
      params.require(:flowchart).permit(:name, :content, :zoomlvl, :approved, :widthp)
    end
end
